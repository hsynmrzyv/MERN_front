import React from "react";

interface SearchPropTypes {
  className?: string;
}

const Search = (props: SearchPropTypes) => {
  return (
    <svg
      width="20"
      height="21"
      viewBox="0 0 20 21"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={props.className}
    >
      <path
        d="M9.22523 16.9799C11.28 16.9799 13.2507 16.1636 14.7037 14.7106C16.1566 13.2577 16.9729 11.287 16.9729 9.23219C16.9729 7.17737 16.1566 5.20672 14.7037 3.75374C13.2507 2.30077 11.28 1.4845 9.22523 1.4845C7.17041 1.4845 5.19976 2.30077 3.74679 3.75374C2.29381 5.20672 1.47754 7.17737 1.47754 9.23219C1.47754 11.287 2.29381 13.2577 3.74679 14.7106C5.19976 16.1636 7.17041 16.9799 9.22523 16.9799Z"
        stroke="#878A92"
        strokeWidth="1.44"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M18.5225 19.5155L14.5782 15.5712"
        stroke="#878A92"
        strokeWidth="1.44"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
};

export default Search;
